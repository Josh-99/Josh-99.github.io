...
const mountains = [{
    name: 'Arnison Crag',
    image: ['./Images/arnison.jpg', './Images/arnison 2.jpg', './Images/arnison 3.jpg'],
    imageNumber: 0,
    region: 'Eastern',
    height: 1424,
    averagetime: '2 Hours',
...
{
    name: 'Blencathra',
    image: ['./Images/blencathra.jpg', './Images/blencathra 2.jpg', './Images/blencathra 3.jpg'],
    imageNumber: 0,
    region: 'Northern',
    height: 2847,
...
{
    name: 'Scafell Pike',
    image: ['./Images/scafell-pike.jpg', './Images/scafell-pike 2.jpg', './Images/scafell-pike 3.jpg'],
    imageNumber: 0,
    region: 'Southern',
    height: 3210,
...
{
    name: 'Yewbarrow',
    image: ['./Images/yewbarrow.jpg', './Images/yewbarrow 2.jpg', './Images/yewbarrow 3.jpg'],
    imageNumber: 0,
    region: 'Western',
    height: 2058,
...
doneBtn.addEventListener('click', function () {
    ...
    const searchMountains = mountains.filter(mountain => {
        ...
        else if (filterArray.includes('Not Complete') && filterArray.includes('Complete')) {
            return mountain;
...
const displayMountainsFunction = (searchMountains = mountains) => { //expecting searchMoutains but if it doesn't get seacrMontains it will default to mountains and show all within the array
    let displayMountains = searchMountains.map(mountain => { //map used to map info from array to HTML input
        ...
        <div class ='mountain-border ${mountain.complete === 'Complete' ? 'show-mountain-border' : ''}'>
        <div class='name'>${mountain.name}</div>
        <div class='image-container'>
            <i id='${mountain.name}-left'class="fas fa-angle-left"></i>
            <img src=${mountain.image[0]} id='${mountain.name}-image' class='mountain-image' alt=${mountain.name}>
        ...
        <div class='mountain-info'>
            ...
            <li><form class='completion-status'>
                ...
                ${mountain.complete === 'Complete' ? `<input class='${mountain.name} checkbox' type='checkbox' alt='mountain complete checkbox' checked >  ` : `<input class='${mountain.name} checkbox' type='checkbox' alt='mountain complete checkbox' > `}
            </form>
            </li>
            </ul>
...
sort.addEventListener('change', function (e) {
    ...
    const sortMostDifficult = mountains.sort((a, b) => {
        if (e.target.value === "Difficulty: High to Low" && a.difficulty > b.difficulty) {
            return -1
        } else {
            return 1;
        }
    ...
    const sortLeastDifficult = mountains.sort((a, b) => {
        if (e.target.value === "Difficulty: Low to High" && a.difficulty < b.difficulty) {
            return -1
        } else {
            return 1;
        }
...
clickRight.forEach(click => {
    click.addEventListener('click', function (e) {
        ...
        let findMountainImageNumber = mountains.find(mountain => {
            console.log(mountain.name, mountainName)
            if (mountain.name === mountainName && mountain.imageNumber >= findMountainImage.length - 1) {
